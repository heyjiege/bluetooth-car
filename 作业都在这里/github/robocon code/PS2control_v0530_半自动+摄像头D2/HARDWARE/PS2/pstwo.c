...
/**********编写多个函数，在void PS2_SetInit(void)中调用。***********/
//向手柄发送命令
void PS2_Cmd(u8 CMD)
{
	volatile u16 ref=0x01;
	Data[1] = 0;
	for(ref=0x01;ref<0x0100;ref<<=1)  //左移16次 
	{
		if(ref&CMD)
		{
			DO_H;                   //输出一位控制位
		...
		DELAY_TIME;
		CLK_H;
		if(DI)                  //#define DI   PBin(12)  为 PB12 输入
			Data[1] = ref|Data[1];
	}
	delay_us(16);
}
...
u8 PS2_DataKey()
{
	...
	{	    
		if((Handkey&(1<<(MASK[index]-1)))==0)  //MASK[index]-1  ？
		return index+1;
	}
	return 0;          //没有任何按键按下
}
...
//short poll   //建立PC机与手柄的连接
void PS2_ShortPoll(void)
{
	CS_L;
	delay_us(16);
	PS2_Cmd(0x01);   //向手柄发送“0x01”
	PS2_Cmd(0x42);  
	PS2_Cmd(0X00);
	PS2_Cmd(0x00);
	PS2_Cmd(0x00);
